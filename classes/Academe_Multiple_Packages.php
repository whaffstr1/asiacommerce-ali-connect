...
class Academe_Multiple_Packages
{
        ...
        foreach ($order->get_items() as $order_line_id => $product) {
            if (isset($product['product_id']) && in_array($product['product_id'], $product_ids)) {
                ...
                if(get_current_blog_id()==2421){
                    $meta_ali_product = get_post_meta($product['product_id'], 'ali__is_ali_product', true);
                    wc_add_order_item_meta($order_line_id, 'ali__is_ali_product', $meta_ali_product, true);                    
                }
    ...
    public function generate_packages($packages)
    {
        //if ($this->enabled) {
            ...
            $multi_packages_type = 'product-meta';
            ...
            elseif ($multi_packages_type == $product_meta_prefix) {
                ...
                foreach (WC()->cart->get_cart() as $item) {
                    if ($item['data']->needs_shipping()) {
                        ...
                        $meta_ali_product = get_post_meta($product_id, 'ali__is_ali_product', true);

                        if($meta_ali_product!='1'){
                            $meta_value = 'lokal';
                        }
                        ...
                        if($meta_value!='lokal')
                            ...
                            $package_meta['package_grouping_value'] = 'lokal';
